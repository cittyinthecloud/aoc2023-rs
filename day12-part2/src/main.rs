...
fn do_aoc(input: &str) -> usize {
    input.lines().map(|line| {
        let (orig_line, spans_str) = line.split_once(' ').unwrap();
        let mut line: String = String::with_capacity(orig_line.len()*5+4);
        for _ in 0..4 {
            line.push_str(orig_line);
            line.push('?')    
        }
        line.push_str(orig_line);

        let mut spans: Vec<usize> = spans_str.split(',').map(|x| x.parse::<usize>().unwrap()).collect();
        // copy and copy and copy and copy and o
        let spans_1 = spans.clone();
        spans.extend(spans.clone());
        spans.extend(spans.clone());
        spans.extend(spans_1);
